...
export function ShippingAddressForm({
  ...
  useEffect(() => {
    if (isOpen && address) {
      ...
      axios
        .get(`/api/check-shipping-address?wallet=${address.toString()}`)
        .then((response) => {
  ...
  const submitShippingInfo = async (
    ...
    if (useExisting) {
      if (!addressId) {
        setSubmitError(
          t("shipping.selectAnAddress") || "Please select an address"
        );
      ...
      if (!formData.agreeToTerms) {
        setSubmitError(
          t("shipping.mustAgreeToTerms") || "You must agree to terms"
        );
        setIsSubmitting(false);
        return;
      }
          ...
          <div className="p-4">
            <div className="bg-[#1A1A1A] rounded-lg p-4 mb-4">
              <h3 className="font-bold text-white mb-2">
                {t("shipping.existingAddresses") || "Existing Addresses"}
              </h3>
            ...
            <div className="flex flex-col gap-2 mt-4">
              ...
              >
                {isSubmitting
                  ? t("common.processing")
                  : t("shipping.useThisAddress") || "Use This Address"}
              </Button>
              <Button
                className="w-full"
                variant="secondary"
            ...
            {existingAddresses.length > 0 && (
              <div className="mb-4">
                ...
                >
                  ...
                  {t("shipping.backToExistingAddresses") ||
                    "Back to existing addresses"}
                </Button>
              </div>
            )}
