...
export function useTransactions(initialLimit = 10) {
  ...
  // Memoize formatted transaction to avoid unnecessary re-calculations
  const formatTransaction = useCallback((tx: any, index: number): Transaction => {
  ...
  return useMemo(() => ({
    ...
    refreshTransactions,
    loadMore,
  }), [transactions, isLoading, isLoadingMore, error, hasMore, refreshTransactions, loadMore]);
}
